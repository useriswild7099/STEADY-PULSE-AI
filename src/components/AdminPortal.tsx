...
export function AdminPortal() {
    ...
    useEffect(() => {
        if (selectedClient) {
            generateLeadGenPrompt(selectedClient);
            generateContentPrompt(selectedClient);
            generateMissingAnalysis(selectedClient);
            generateUspStatement(selectedClient);
        }
    }, [selectedClient]);

    const generateLeadGenPrompt = (client: any) => {
        const b = client.onboardingData?.generalData;
        const d = client.onboardingData?.brandData;
        
        if (!b) return setLeadGenPrompt("No business data available to generate prompt.");

        const prompt = `ROLE: You are a LinkedIn lead generation strategist for ${b.companyName || 'this client'}.
...
OUTPUT: Provide a detailed, actionable 90-day LinkedIn growth plan with specific tactics, templates, and KPIs.`;
        setLeadGenPrompt(prompt);
    };

    const generateContentPrompt = (client: any) => {
        const b = client.onboardingData?.generalData;
        const d = client.onboardingData?.brandData;

        if (!d) return setContentPrompt("No brand DNA data available to generate prompt.");

        const prompt = `ROLE: You are a LinkedIn content strategist for ${b?.companyName || 'Client'}.
...
OUTPUT: Full content calendar with post copy, carousel outlines, and engagement playbook.`;
    ...
    const generateMissingAnalysis = (client: any) => {
        const b = client.onboardingData?.generalData;
        const d = client.onboardingData?.brandData;
        
        if (!b && !d) return setMissingAnalysis("No data available for analysis.");
    ...
    const generateUspStatement = (client: any) => {
        const b = client.onboardingData?.generalData;
        const d = client.onboardingData?.brandData;

        if (!b || !d) return setUspStatement("Insufficient data to generate USP.");

         const usp = `UNIQUE VALUE PROPOSITION
                    ...
                    <div className="w-full max-w-6xl h-[90vh] bg-[#0E1217] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-scaleUp">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 bg-black/20 custom-scrollbar">
                         ...
                         <div className="p-4 border-t border-white/10 bg-[#0E1217] flex justify-end gap-3">
                             ...
                             <button className="px-6 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold shadow-lg shadow-blue-600/20 transition-all">
                                Save Changes
                             </button>
                         </div>
                    </div>
